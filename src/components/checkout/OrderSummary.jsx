...
const OrderSummary = () => {
    ...
    return (
        <div className='p-5 lg:p-10 bg-[#F4F4F4] rounded-xl lg:rounded-3xl'>
            <h4 className='font-poppins font-semibold text-2xl leading-7.5 text-secondary mb-7'>Order Summary</h4>
            <ul className='font-montserrat font-bold text-base lg:text-xl leading-7.5 text-secondary'>
                {orderSummaryData.map((item, index) => (
                    <li key={index} className='flex lg:py-5 lg:pr-11 border-b border-[#C3C3C3] max-sm:flex-wrap max-sm:gap-y-4 max-sm:justify-between'>
                        <p className='lg:w-78 lg:pr-15.5 font-normal'>{item.productName}</p>
                        <span className='lg:w-18'><span className='sm:hidden font-normal'>Quantity: </span>{item.quantity}</span>
                        <p><span className='sm:hidden font-normal'>Price: </span>${new Intl.NumberFormat('en-US').format(item.price)}.00</p>
                    </li>
                ))}
            </ul>
            <div className='flex max-sm:justify-between lg:gap-x-63.5 lg:pr-11 font-montserrat font-normal text-base lg:text-xl leading-7.5 text-secondary mt-5 mb-6 lg:mb-12'>
