...
const OrderSummary = () => {
    ...
    return (
        <div className='sm:max-md:max-w-68 md:max-lg:max-w-80 p-5 lg:p-10 bg-[#F4F4F4] rounded-xl lg:rounded-3xl'>
            ...
            <ul className='font-montserrat font-bold text-base sm:max-md:text-sm lg:text-xl  lg:leading-7.5 text-secondary'>
                {orderSummaryData.map((item, index) => (
                    <li key={index} className='flex lg:py-5 lg:pr-11 border-b border-[#C3C3C3] max-lg:flex-wrap max-lg:gap-y-4 max-lg:justify-between'>
                        <p className='max-lg:w-full lg:w-78 lg:pr-15.5 font-normal'>{item.productName}</p>
                        <span className='lg:w-18'><span className='lg:hidden font-normal'>Quantity: </span>{item.quantity}</span>
                        <p><span className='sm:hidden font-normal'>Price: </span>${new Intl.NumberFormat('en-US').format(item.price)}.00</p>
                    </li>
                ))}
            </ul>
            <div className='flex max-lg:justify-between lg:gap-x-63.5 lg:pr-11 font-montserrat font-normal text-base lg:text-xl leading-7.5 text-secondary mt-5 mb-6 lg:mb-12'>
            ...
            <div className='py-3 lg:py-5 px-4 lg:px-6 rounded-[10px] bg-white'>
                <div className='flex max-lg:justify-between lg:gap-x-80 font-montserrat font-bold lg:text-xl leading-7.5 text-secondary mb-3 lg:mb-6'>
