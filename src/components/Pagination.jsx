    ...
    return (
        <div className='flex justify-center items-center lg:gap-x-9'>
            ...
            <div className='flex justify-center items-center lg:gap-x-7.5'>
                <button onClick={() => onPageChange(1)} className={`size-8 lg:size-12 flex justify-center items-center rounded-sm font-poppins font-semibold text-lg lg:text-xl leading-7.5 text-secondary cursor-pointer ${currentPage == 1 ? 'bg-primary text-white' : 'hover:bg-gray-200'}`}>1</button>
                {pageNumbers.map((pageNumber, index) => (
                    <button key={index} disabled={pageNumber == '...'} onClick={() => onPageChange(pageNumber)} className={`size-8 lg:size-12 flex justify-center items-center rounded-sm font-poppins font-semibold text-lg lg:text-xl leading-7.5 text-secondary  ${currentPage == pageNumber ? 'bg-primary text-white cursor-pointer' : pageNumber != '...' && 'cursor-pointer hover:bg-gray-200'}`} >{pageNumber}</button>
                ))}
                <button onClick={() => onPageChange(totalPages)} className={`size-8 lg:size-12 flex justify-center items-center rounded-sm font-poppins font-semibold text-lg lg:text-xl leading-7.5 text-secondary cursor-pointer ${currentPage == totalPages ? 'bg-primary text-white' : 'hover:bg-gray-200'}`}>{totalPages}</button>
            </div>
            <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className='size-8 lg:size-12 flex justify-center items-center rounded-sm cursor-pointer hover:bg-gray-200 mx-1 lg:mx-2 disabled:hover:bg-transparent disabled:opacity-20 disabled:cursor-not-allowed'><ArrowDownIcon className='size-5 lg:size-8' rotate='270deg' /></button>
        </div>
    )
}
